...
setwd("~/Documents/Swapnil/Data Science/R_Projects/week4/Getting and Cleaning Data Course Project")
...
feature_vector_train <- read.table("X_train.txt")
...
train_dataframe <- data.frame(subject_train, activity_train, feature_vector_train)
...
test_dataframe_df <- tbl_df(test_dataframe)
train_dataframe_df <- tbl_df(train_dataframe)

## Below part of the scripts combines test and train data to generate complete_data
##It has 10299 rows X 563 columns
complete_data <- bind_rows(test_dataframe_df, train_dataframe_df)
...
extract_dataframe <- tbl_df(extract_dataframe)
...
summary_table <- extract_dataframe %>%
                mutate(activity_name = case_when(
                        ...
                        activity == 5 ~ "STANDING",
                        activity == 6 ~ "LAYING")) %>%
        relocate(activity_name, .after = activity) %>%
